  ...
  return (
    <div className='RDRE__cat'>
      <div className="custom-container">
        ...
        <div className="row">
            {products.filter(item => item.category === category).sort((a, b) => {
              if (!sort) {
                return 0;
              ...
              return (
                <div className="col-3" key={index}>
                  <Link to={`/shop/products/details/${product._id}`}>
                    <div className="card mb-4" style={{ width: "100%",height:"95%" }}>
                      <img src={product.image} className="card-img-top" alt="..." />
                      <div className="card-body" style={{ padding: "20px" }}>
                        <h3 className="card-title">{product.title}</h3>
                        <p style={{fontSize:"16px",color:"#e8e8e8"}}>€{product.price}</p>
                      </div>
                    </div>
                  </Link>
                </div>
